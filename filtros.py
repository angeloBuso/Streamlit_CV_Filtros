...
def filtros():
    # inicialização da figura da imagem
    ...
    if escolha == "Filtros":
        ...
        # usando o PIL, pois com openCV há a questão de inversão do RGB -> BGR
        ...
        # usando um "radio" em sidebar, aparecem as opções da lista passada um botão para selecionar
        ...
        if filtros == 'GrayScale':
            # objeto imagem é do tipo PilImage, logo tenho que converter ela pra RGB
            converter_imagem = np.array(our_imagem.convert('RGB'))
            # converto imagem para formato do OpenCV
            ...
            # o blur é aplicado em todos os canais, independente se é RGB para BGR
            ...
            # objeto imagem é do tipo PilImage, logo tenho que converter ela pra RGB
            converter_imagem = np.array(our_imagem.convert('RGB'))
            # converto a imagem para formato BGR (formato do OpenCV)
            ...
            # controles de parametros de kernel de area, substitui no paramento do kernel (input usuário)
            # espera receber numeros impares
            b_amount = st.sidebar.slider("Kernel (n x n)", 3, 81, 9, step=2)
            # objeto imagem é do tipo PilImage, logo tenho que converter ela pra RGB (não aceita BGR direto)
            converter_imagem = np.array(our_imagem.convert('RGB'))
            # converto a imagem para formato BGR (formato do OpenCV)
            converter_imagem = cv2.cvtColor(converter_imagem, cv2.COLOR_RGB2BGR)
            ...
            # a biblioteca Pil permite usar contraste, brilho de forma direta
